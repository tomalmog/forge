...
def run_training(
    ...
    metrics = _run_training_loop(context)
    return _persist_training_outputs(context, metrics)
...
def _build_runtime_context(
    ...
    tokenizer = _fit_tokenizer(records)
...
def _persist_training_outputs(
    ...
    history_path = save_training_history(context.output_dir, metrics)
    plot_path = _try_save_plot(context.output_dir, metrics)
...
def _fit_tokenizer(records: list[DataRecord]) -> VocabularyTokenizer:
...
def _validate_training_options(options: TrainingOptions) -> None:
...
def _run_training_loop(context: TrainingRuntimeContext) -> list[EpochMetric]:
...
def _run_default_training_loop(context: TrainingRuntimeContext) -> list[EpochMetric]:
    """Run the built-in epoch training loop."""
    metric_rows: list[EpochMetric] = []
    for epoch_index in range(1, context.options.epochs + 1):
        train_loss = _run_epoch_pass(context, context.train_batches, training=True)
        validation_loss = _run_epoch_pass(
            context,
...
def _try_save_plot(output_dir: Path, metrics: list[EpochMetric]) -> Path | None:
    """Save training plot, skipping when matplotlib is missing."""
    try:
        return save_training_plot(output_dir, metrics)
